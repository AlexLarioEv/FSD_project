...
describe("AppImage", () => {
    ...
    test("показывает загрузочный фолбэк, пока изображение загружается", () => {
        mockImageLoad();
        componentRender(<AppImage src="valid-image.jpg" fallbackLoading={<div data-testid="loading">Загрузка...</div>} />);
    ...
    test("показывает фолбэк ошибки, если изображение не загрузилось", async () => {
        ...
        componentRender(<AppImage src="invalid-image.jpg" fallbackError={<div data-testid="error">Ошибка загрузки</div>} />);

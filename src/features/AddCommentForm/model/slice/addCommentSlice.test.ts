...
describe('addCommentSlice', ()=>{
    ...
    test('setComment', () => {
        expect(addCommentReducer(state, addCommentActions.setComment('action text'))).toEqual({text: 'action text', error: 'error'});
    });

    test('state undefined', () => {
        expect(addCommentReducer(undefined, addCommentActions.setComment(''))).toEqual({text: ''});
    });
});

...
export const ArticleViewSelector: FC<TArticleViewSelectorProps> = ({ className, view = EArticleView.SMALL, onViewClick }) => {
    ...
    const views = useMemo(()=>viewTypes.map((element, index)=>(
        <Button 
            key={index} 
            theme={EButtonTheme.CLEAR}
            onClick={handleClickView(element.view)}
        >
            <Icon className={classNames('', {[styles.notSelected]: view !== element.view})} Svg={element.icon}/>
        </Button>)), [handleClickView, view])
